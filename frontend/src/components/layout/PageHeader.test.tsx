...
describe('PageHeader', () => {
  ...
  it('renders help link when helpUrl is provided', () => {
    ...
    const helpLink = screen.getByRole('link', { name: /Open the Artha wiki/i });
  ...
  it('does not render help link when helpUrl is omitted', () => {
    render(<PageHeader title="My Page" />);
    expect(screen.queryByRole('link', { name: /Open the Artha wiki/i })).not.toBeInTheDocument();
  });

  it('shows a tooltip describing the wiki destination', () => {
    render(<PageHeader title="My Page" helpUrl="https://example.com/help" />);
    expect(
      screen.getByRole('tooltip', { name: /Open the Artha wiki/i }),
    ).toBeInTheDocument();
  });
  ...
  it('renders help link alongside action buttons when both are provided', () => {
    ...
    expect(screen.getByRole('link', { name: /Open the Artha wiki/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Add New' })).toBeInTheDocument();
  });
