...
describe('SeasonalityPanel', () => {
  ...
  it('draws spending months with the theme accent rather than red', () => {
    ...
    // Guard against both mistakes this panel's payee sibling has made:
    // hardcoded Tailwind shades ignore the active colour theme, and red
    // belongs to the Monthly Totals chart rather than to a twelve-month
    // magnitude comparison.
    expect(container.innerHTML).not.toMatch(/bg-(red|green)-\d/);
    expect(container.innerHTML).not.toMatch(/text-(red|green)-\d/);
    expect(container.innerHTML).not.toContain('var(--chart-expense)');
  });

  it('keeps inflow months green so the sign survives', () => {
    // A category that refunds more than it costs in July.
  ...
  it('draws a month with no history as an empty slot, not a zero bar', () => {
    // Only May has history; the other eleven months get the hairline marker.
    const { container } = renderPanel([month('2025-05', -50), month('2026-05', -50)]);
    const bars = container.querySelectorAll('li > div > div[style]');
    expect(bars.length).toBe(1);
    expect(container.querySelectorAll('li .h-px').length).toBe(11);
  });
