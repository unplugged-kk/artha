 ...
 * with a structured `artha://<entity-type>/<uuid>` href; this module is the
...
const ENTITY_URI_REGEX =
  /^artha:\/\/(account|payee|category|transaction|security|scheduled)\/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$/i;

export function isArthaHref(href: string | undefined): boolean {
  return typeof href === 'string' && /^artha:/i.test(href);
}
 ...
 * `[Groceries](artha://category/...)` markup would otherwise leak into the UI.
 */
...
/**
 * Map a `artha://` entity URI to its in-app route, or null when the URI is
