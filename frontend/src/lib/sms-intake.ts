...
export const smsIntakeApi = {
  parse: async (dto: ParseSmsDto): Promise<ParsedSmsResponse> => {
    const response = await apiClient.post<ParsedSmsResponse>('/import/sms/parse', dto);
    return response.data;
  },

  import: async (dto: ImportSmsDto): Promise<SmsImportResult> => {
    const response = await apiClient.post<SmsImportResult>('/import/sms/import', dto);
  ...
  getKnownSenders: async (): Promise<KnownBankSender[]> => {
    const response = await apiClient.get<KnownBankSender[]>('/import/sms/senders/known');
    return response.data;
  },
};
