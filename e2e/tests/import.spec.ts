...
test.describe("Import Transactions", () => {
  ...
  test("imports a QIF file end to end", async ({ authedPage: page, api }) => {
    ...
    await page
      .getByLabel(/import into account/i)
    ...
    await expect(page.locator("li", { hasText: /imported:/i })).toContainText(
      "1",
    );
  });

  test("skips duplicate transactions when re-importing the same QIF file", async ({
    authedPage: page,
    api,
  }) => {
    const account = await createAccount(api, {
      name: `Dedup Target ${uniqueId()}`,
    });
    const qif = [
      "!Type:Bank",
      "D05/25/2026",
      "T-55.00",
      "PE2E Duplicate Test Payee",
      "^",
      "",
    ].join("\n");
    ...
    // Second import of the same file into the same account
    await page.goto("/import");
    await page.locator('input[type="file"]').setInputFiles(filePayload);
    await expect(
      page.getByRole("heading", { name: /select destination account/i }),
    ...
    await expect(page.locator("li", { hasText: /skipped:/i })).toContainText(
      "1",
    );
  });
});
