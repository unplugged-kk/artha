...
|---------|---------|---------|
| `DATABASE_USER` / `DATABASE_PASSWORD` | Owner role (init, migrate, seed) | `artha_user` |
...
| `enforce` | yes | `artha_app` | Policies live **on tables where RLS is enabled**. Before the enable migration ships (flip B), this only drops privileges (flip A) and row visibility is unchanged. |
...
| Backup restore | Normal user context + `preserveTimestamps: true` context flag; `withScopedDb()` adds `app.preserve_timestamps = 'on'` to each restore transaction (replaces the old `ALTER TABLE ... DISABLE TRIGGER` DDL, which `artha_app` cannot run). |
...
4. **Production flip A: privilege drop** — set `RLS_MODE=enforce` while the enable migration is **not** yet deployed to prod. The runtime becomes `artha_app`; no table has RLS enabled, so row visibility is unchanged. Only privilege bugs can surface (loud `permission denied` errors, e.g. a missed grant or a DDL path). Soak. Revert: `RLS_MODE=shadow`.
...
- **Permission errors** — `grep 'permission denied for' <pg log>`; a hit means a missing GRANT or a privileged operation (DDL) attempted as `artha_app`.
...
```sql
-- Connect as artha_app (or: SET LOCAL ROLE artha_app inside a transaction as a superuser)
...
1. Remove `DATABASE_APP_PASSWORD` (and set `RLS_MODE=off`) from the environment, then redeploy so nothing reconnects as `artha_app` — and so db-init does not simply recreate the role on the next restart. On CNPG, also remove the `managed.roles` entry.
2. Terminate any straggling sessions:
   ```sql
   SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE usename = 'artha_app';
   ```
...
3. Run the down-SQL as the owner. Drop policies keyed on **`pg_policies`, not on which tables have RLS enabled** — the rollout deliberately creates a state where policies exist on every user-owned table (53 when M3 shipped) while zero are enabled (pre-flip-B), and `DROP FUNCTION` fails with a dependency error while any policy still references the helpers:
   ...
   REVOKE ALL ON ALL TABLES IN SCHEMA public FROM artha_app;
   REVOKE ALL ON ALL SEQUENCES IN SCHEMA public FROM artha_app;
   REVOKE USAGE ON SCHEMA public FROM artha_app;
   -- Run as the owner; no FOR ROLE clause (binds to the current role, whatever its name):
   ALTER DEFAULT PRIVILEGES IN SCHEMA public REVOKE ALL ON TABLES FROM artha_app;
   ALTER DEFAULT PRIVILEGES IN SCHEMA public REVOKE ALL ON SEQUENCES FROM artha_app;
   DROP ROLE IF EXISTS artha_app;
   ```
...
| A cron job suddenly processes nothing | The job runs context-less under `artha_app` | Wrap its cross-user query in `withSystemContext` and per-user work in `withUserContext`. |
...
| Backup restore fails with `ERROR: must be owner of table ...` | Restore still uses the old `ALTER TABLE ... DISABLE TRIGGER` DDL, which `artha_app` cannot run | Use the `app.preserve_timestamps` GUC path (the trigger function honors it); no DDL at restore time. |
...
| `permission denied for table ...` (not a row error) | Missing GRANT to `artha_app` | Restart the backend — db-init re-applies grants + default privileges idempotently on every startup. Only tables created by a role other than the owner escape the default privileges. |
...
3. Grants are automatic via the default privileges db-init maintains **only** if the owner role created the table; verify `artha_app` has DML on it otherwise (a backend restart re-applies grants).
