...
| Backup **restore** runs DDL -- `ALTER TABLE ... DISABLE TRIGGER "update_*_updated_at"` -- to preserve restored `updated_at` values | `backend/src/backup/backup.service.ts` (~line 1317) | `ALTER TABLE` requires table **ownership**; neither `app.bypass_rls` nor DML grants help. Restore breaks under `artha_app` unless the trigger function is made GUC-aware (Phase 3) or restore gets an owner DataSource (Phase 4). |
...
1. **Two roles.** Keep `artha_user` (superuser/owner) for DDL, migrations, seed, and privileged work. Add `artha_app` (LOGIN, **not** superuser, **not** owner, **no** `BYPASSRLS`) as the runtime role. Privilege and row-visibility are orthogonal in Postgres -- `artha_app` gets table DML grants but RLS still filters its rows.
...
6. **Phased rollout with one mode flag and two independent flips.** A single `RLS_MODE=off|shadow|enforce` enum (no boolean pair, so no invalid combination is representable). Flip A switches the runtime role to `artha_app` while **no table has RLS enabled** -- only privilege bugs (`permission denied`, DDL) can surface, visibility unchanged. Flip B ships the enable migration -- only context bugs (zero rows) can surface. Each flip reverts instantly by setting `RLS_MODE=shadow` (the owner role bypasses RLS even on enabled tables).
...
```sql
-- db-init first runs, parameterized: SELECT set_config('artha.app_password', $1, false)
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'artha_app') THEN
    EXECUTE format('CREATE ROLE artha_app LOGIN PASSWORD %L', current_setting('artha.app_password'));
  ELSE
    EXECUTE format('ALTER ROLE artha_app PASSWORD %L', current_setting('artha.app_password'));
  END IF;
END $$;
```
...
```sql
GRANT USAGE ON SCHEMA public TO artha_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO artha_app;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO artha_app;
-- future tables created by the owner (migrations) are auto-granted:
ALTER DEFAULT PRIVILEGES IN SCHEMA public
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO artha_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public
  GRANT USAGE, SELECT ON SEQUENCES TO artha_app;
```

   **No `FOR ROLE` clause.** Without it, `ALTER DEFAULT PRIVILEGES` applies to the *current* role -- the actual owner, whatever its name. The owner role name is operator-chosen (`docker-compose.prod.yml` passes `${POSTGRES_USER}` with no default; CI uses `artha_test`; `artha_user` is only the `.env.example` sample), so a hardcoded `FOR ROLE artha_user` would either fail outright or -- worse -- silently register default privileges for a role that never creates tables. Re-applying grants at every startup also closes the late-provisioning gap: an operator who sets the password (or adds the CNPG role) *after* upgrading converges on the next restart; there is no "the grants migration already ran" trap.
...
- `db-init.ts`, `db-migrate.ts`, and the seed entrypoint keep using `DATABASE_USER` (owner). The split is **by process**: startup scripts = owner; long-running API = `artha_app`.
...
| Raw SQL path forgets to scope by user | Yes | Same -- the policy applies to every statement the `artha_app` role runs. |
...
3. `docker compose -f docker-compose.dev.yml up`: register two users; confirm normal app behavior (accounts, transactions, reports, budgets) with the runtime on `artha_app`.
4. Manual DB proof: `psql` as `artha_app`, `SET app.current_user_id = '<userA>'; SELECT count(*) FROM transactions;` -> only userA's count; set to userB -> userB's; `RESET app.current_user_id; SELECT count(*) FROM transactions;` -> `0`.
