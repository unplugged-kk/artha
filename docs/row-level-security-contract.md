...
| Table | Why exempt |
|---|---|
| `broker_import_layouts` | Global reference data describing the broker/bank file layouts this build knows how to parse: broker, layout, version, the parser hook that reads it, and the golden fixture it is proven against. A layout is a property of the source export, not of the household reading it -- one Zerodha tradebook layout serves every user, and the registry is resolved by parser code, not by query. There is no owner column to policy on, and a per-user copy would let two users disagree about how the same file parses, which is the one thing a versioned layout registry exists to prevent. Seeded from code alongside the parsers (Phase 4); nothing user-identifiable is stored. |
...
| `instrument_aliases` | Exchange ticker renames: `(exchange, alias_symbol)` is the same instrument as `canonical_symbol` (`TATAMOTORS -> TMPV` on NSE, `ZOMATO -> ETERNAL`, `FB -> META`). The rename is a fact about the venue, identical for every user, so there is no owner column to policy on and it is not indirect either -- there is no parent row to resolve ownership through. A per-user copy would let two users hold two different answers about the same instrument. Read by the provider-key mapping (`securities/providers/instrument-key.util.ts`) through a resolver; written by operators/seeding, never by a request in a user's context. |
