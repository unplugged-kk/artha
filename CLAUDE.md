...
**The scheme is shared; the shard key and the shape are not.** Automatic backups shard by *user* id with the id as a directory (`<base>/<ab>/<cd>/<userId>/artha-backup-daily-2026-08-03.json.gz`), because the filename carries only tier and date and a flat folder let one user's retention pass delete another's files. Local attachments shard by *attachment* id with the id as the file itself, because that id is already globally unique.
